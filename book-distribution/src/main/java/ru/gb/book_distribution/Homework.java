...
public class Homework {
    ...
    * */
    ...
    /*
    * 1. Подключить OpenApi 3 и swagger к пректу библиотека.
    * 2. Описать все контроллеры, эндпоинты и возврощаемые тела, с помощью анатаций OpenApi 3.
    * 3. В качестве результата, необходимо прислать скриншот(ы) страницы swagger (с ручками)
    * ** Скриншоты в папке  screenshot/
    * */

    /*
    * 1. Для HTML- страниц реализовать авторизацию, через login-форму.
    * 2.1* Реализовать таблицы User(id, name, password) и Rol(id, name), связанные многие ко многим
    * 2.2* Реализовать UserDetailsService, который предстовляет данные из базы данных(таблицы User и Role)
    * */
